...
$script:XUnitDefault = 'xunit.console.exe'
...
function Run-Tests {
	...
	if($SummaryPath -ne $null) {
		Create-Directory $SummaryPath
	}
	
  Set-XUnitPath (Join-Path (Get-Xunit-Directory) $XUnitDefault)
	Invoke-XUnit -Path $Path -TestSpec $TestSpec -SummaryPath $SummaryPath\nunit.xml
}

function Get-Xunit-Directory
{
  Resolve-Path "..\packages\xunit.runner.console.2.0.0\tools"
...
function Get-XUnitPath
{
  ...
  $parentDirectory = Split-Path (Split-Path (Get-Xunit-Directory))
...
function New-XUnitProjectFile
{
  ...
  $testAsms = @()
  $uniqueFiles = @{}
  $Path | Get-ChildItem -Recurse -Include $TestSpec -Exclude *Integration.Tests.dll* |
  ...
  $projectPath = ($tests_dir + '\merged.xunit')
  
  $xunittemplate = "<?xml version=""1.0"" encoding=""utf-8""?>
<xunit>
  <assemblies>
    <assembly filename=""""><output type=""nunit"" filename="""" shadow-copy=""false"" /></assembly>
  </assemblies>
</xunit>
"

  $xml = [XML] $xunittemplate
...
function Get-TestSummary
{
  ...
  $errors = $xDoc.SelectNodes("//test-suite/failure")
  $successes = $xDoc.SelectNodes('//test-case[@success="True"]')
  $failures = $xDoc.SelectNodes('//test-case[@success="False"]')
  
  $tr = $xDoc.'test-results'

  return @{
    total = [Convert]::ToInt32($successes.Count + $failures.Count);
    errors = [Convert]::ToInt32($errors.Count);
    failures = [Convert]::ToInt32($failures.Count);
...
function New-MergedNUnitXml
{
  ...
  if ($xml.'test-results'.errors -ne '0')
  {
    $errors = $xml.'test-results'.errors
    throw $errors + " unit test fixtures failed to run."
  }
...
Export-ModuleMember -Function Set-XUnitPath, New-XUnitProjectFile, Invoke-XUnit,  New-MergedNUnitXml
